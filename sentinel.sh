...
# --- CONFIGURATION ---
VPN_PATH="$HOME/Desktop/tools/bosatek_sentinel/THM.ovpn"
...
# Pane 0: Master Control / Active Shell
tmux send-keys -t $SESSION:0 "export IP=$TARGET_IP; clear" C-m

# Pane 1: VPN (Split Right)
tmux split-window -h -t $SESSION:0
tmux send-keys -t $SESSION:0.1 "sudo openvpn $VPN_PATH" C-m

# Pane 2: Tactical AI (Split Bottom Right)
tmux split-window -v -t $SESSION:0.1
tmux send-keys -t $SESSION:0.2 "ollama run bosatek-sentinel" C-m
tmux send-keys -t $SESSION:0.2 "Analyze my previous writeups in $NOTES_PATH for initial vectors." C-m

# Pane 3: The Recon/Wordlist Menu (Split Bottom Left)
...
tmux send-keys -t $SESSION:1 "while true; do 
  # Watch for new 200 OKs in hits.json and open in Burp Chromium
